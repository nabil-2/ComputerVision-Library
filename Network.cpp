...
Network::Network() {
	...
	do {
		std::string filename;
		//std::cout << "Datei mit labels: ";
		//std::cin >> filename;
		//inFile.open(filename);
		inFile.open("C:\\Users\\admlocal\\Desktop\\KI\\Files\\CNN\\labels.txt");
		if (!inFile) std::cout << "Datei existiert nicht. Erneut eingeben. ";
		//std::cin >> filename;
	} while (!inFile);
	std::string fileText = "";
	while (!inFile.eof()) {
...
void Network::setActivationFunction(std::string actFunction) {
	...
	if (typeid(*lastLayer) == typeid(Dense)) {
		dynamic_cast<Dense*>(lastLayer)->lastLayer = true;
	} else if (typeid(*lastLayer) == typeid(MultiDense)) {
		dynamic_cast<MultiDense*>(lastLayer)->lastLayer = true;
	} else if (typeid(*lastLayer) == typeid(MultiDense2)) {
		dynamic_cast<MultiDense2*>(lastLayer)->lastLayer = true;
	}
};
...
std::vector<std::vector<std::string>> Network::predict() {
	...
	if (typeid(*lastLayer) == typeid(Softmax)) {
		...
		return encodeOutput(dynamic_cast<MultiDense2*>(lastLayer)->activations);
	}
...
std::vector<denseA> Network::predictVal() {
	...
	if (typeid(*lastLayer) == typeid(Softmax)) {
		...
		return dynamic_cast<MultiDense2*>(lastLayer)->activations;
	}
...
void Network::epoch(std::vector<convA> inputs, std::vector<std::vector<float>> outputs) {
	setInput(&inputs);
	std::vector<denseA> result = predictVal();
	float avgError = 0;
	std::vector<denseA> dCa;
	if (costF == "mean-squared") {
		for (unsigned int i = 0; i < outputs.size(); i++) {
			float err = 0;
			for (unsigned int j = 0; j < outputs[i].size(); j++) {
				err += pow(result[i][j] - outputs[i][j], 2);
			}
			avgError += err;
		}
		avgError /= (float) outputs.size();
		for (unsigned int i = 0; i < outputs.size(); i++) {
			dCa.push_back(denseA());
			for (unsigned int j = 0; j < outputs[i].size(); j++) {
				dCa[i].push_back(2*(result[i][j] - outputs[i][j]));
			}
		}
		std::cout << "error: " + std::to_string(avgError) << std::endl;
	}
	avgE += std::to_string(avgError) + ";";
	std::ofstream outfile("plotData.txt");
	outfile << avgE;
	...
	Layer* lastLayer = level[level.size() - 1][0];
	if (typeid(*lastLayer) == typeid(Dense)) {
		dynamic_cast<Dense*>(lastLayer)->setUpGradient(&dCa);
	} else if (typeid(*lastLayer) == typeid(MultiDense)) {
		dynamic_cast<MultiDense*>(lastLayer)->setUpGradient(&dCa);
	} else if (typeid(*lastLayer) == typeid(MultiDense2)) {
		dynamic_cast<MultiDense2*>(lastLayer)->setUpGradient(&dCa);
	}
	for (unsigned int i = level.size() - 1; i >= 1; i--) {
		level[i][0]->backward();
	}
		...
		for (unsigned int j = 0; j < level[i].size(); j++) {
			...
			if (typeid(*level[i][j]) == typeid(Dense)) {
				...
				readThreads.push_back(std::thread([layer, i, filename] {
					...
					std::cout << "dense " << i << " imported" << std::endl;
				}));
				...
				readThreads.push_back(std::thread([layer, i, filename] {
					...
					std::cout << "conv " << i << " imported" << std::endl;
				}));
				layer->converged = true;
			}
			else if (typeid(*level[i][j]) == typeid(Input)) {
